...
- 小线索、小歌单、小约定、小日子列表使用 `createdAt` 倒序 + `_id` 倒序的稳定 keyset 游标分页（`createdAt` 相同的记录以 `_id` 兜底），保证翻页边界不重复、不遗漏；展示顺序由页面排序规则（置顶、状态等）决定。
- 每个列表必须支持下拉刷新与触底自动加载下一页，加载失败保留已加载内容并允许重试。
- 新增、编辑、删除记录后返回列表，列表通过分页缓存同步立即反映变更，不得被第一页覆盖丢失已加载深度；缓存命中后在后台按旧游标边界重新校验数据新鲜度。
- 筛选视图在自动补页期间必须显示"正在翻找"中间态；连续 4 页未命中后提供手动"继续翻找"入口；只有数据穷尽后才允许显示最终空状态。
- 任务完成进度与小日子总数等统计数字以云端真实总量为准；统计未就绪或失败时显示中性文案，不得用已加载数量冒充全量。
- 小日子"今天就是 / 下一次"摘要必须在列表加载完整后才能展示，未加载完整时保持中性摘要。
