...
所有业务列表（首页回忆时间线、小线索、小歌单、小约定、小日子）必须按需分页加载，不得一次性拉取全部记录，也不得只展示首屏后丢失后续记录。

- 云端单次查询最多返回 20 条，列表以此为一页。
- 小线索、小歌单、小约定、小日子列表按 `createdAt` 倒序分页拉取，展示顺序由页面排序规则（置顶、状态等）决定。
- 每个列表必须支持下拉刷新与触底自动加载下一页，加载失败保留已加载内容并允许重试。
- 新增、编辑、删除记录后返回列表，列表必须刷新出最新内容；未变更时返回列表保留已加载数据。
- 任务完成进度与小日子总数等统计数字以云端真实总量为准，不得只统计已加载的记录。
