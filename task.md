...
- [ ] `removedFiles` 清理成功后再清空；失败时保留可重试数据或写入待清理队列。
...
- [ ] 区分原记录文件、本次新上传文件、待删除旧文件。
- [ ] 新增页创建失败、返回或页面卸载时，回收本次新上传但未关联到记录的文件。
- [ ] 详情页取消编辑、返回或保存失败时，回收本次编辑中新上传且未保存进记录的文件。
- [ ] 避免误删原记录已有文件；`removeEditFile(index, false)` 只应进入待删除旧文件清单。
...
- [ ] 增加共享日期校验：接受空值场景由调用方决定；非空值必须匹配 `YYYY-MM-DD` 且年月日回读一致。
- [ ] `2026-02-29` 在非闰年失败，`2028-02-29` 通过，`2026-02-31` 和 `2026-99-99` 失败。
- [ ] 新增页、详情编辑页、任务编辑页使用共享校验，不再只用正则。
- [ ] 任务仓库排序中的 `validDueDate()` 使用同一校验，避免伪日期参与排序。
...
- [ ] 让 `AppCustomNav` 的返回事件可被页面拦截；页面确认允许后再执行默认返回。
- [ ] 歌曲、任务、备忘编辑页复用已有 `draftDirty`，自定义返回按钮和系统返回都先弹 Wot MessageBox。
- [ ] 详情编辑页补充 dirty 状态或 draft 快照比对，点击“取消”和返回时统一确认。
- [ ] 确认文案使用克制小纸条语气：标题“这张纸条还没收好”，按钮“继续写”“不保存离开”。
...
- [ ] 移除或替换 `本人测试`。
- [ ] 替换时使用符合产品语气且不暴露测试状态的文案，例如“私密”或“小树洞”。
